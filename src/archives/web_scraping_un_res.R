# ============================================================================================ #
#                                                                                              #
#  The script described below shows how to compare the structures of two datasets              #
#  This will be very useful for the data quality checks routine that should be launched        #
#  before starting shiny-applications in order to avoid errors due to data quality             #
#                                                                                              #
#                                                                                              #
#     Input Parameters:                                                                        #
#     ----------------                                                                         #
...
#                                                                                              #
#                                                                                              #
#        (2) lib_location : directory where the packages will be installed                     #
#                                                                                              #
#                                                                                              #
#                                                                                              #
#                                                                                              #
...
#==================================================================
# (3) SCRAPING RESOLUTION FULL TEXT FROM THE RESOLUTION_URL_WEBSITE ####
#==================================================================
...
# sample document per year
df <- resolution_data_fulltext_complete

res_2000 <- which(df$YEAR == 2000)
pos1 <- sample(res_2000, 1)

res_2005 <- which(df$YEAR == 2005)
pos2 <- sample(res_2005, 1)

res_2010 <- which(df$YEAR == 2010)
pos3 <- sample(res_2010, 1)

res_2020 <- which(df$YEAR == 2020)
pos4 <- sample(res_2020, 1)


# get the url of eh considered res
url_global1 <- df$URL[pos1]
url_global2 <- df$URL[pos2]
url_global3 <- df$URL[pos3]
url_global4 <- df$URL[pos4]

# reading the html of the given resolution into R
html_r1 <- read_html(url_global1) %>%
 html_nodes('a') %>%
  html_attr('href') %>%
   grep('EN.pdf', ., value=TRUE) %>%
    str_c("https://digitallibrary.un.org", .)

html_r2 <- read_html(url_global2) %>%
  html_nodes('a') %>%
   html_attr('href') %>%
    grep('EN.pdf', ., value=TRUE) %>%
    str_c("https://digitallibrary.un.org", .)

html_r3 <- read_html(url_global3) %>%
  html_nodes('a') %>%
   html_attr('href') %>%
    grep('EN.pdf', ., value=TRUE) %>%
    str_c("https://digitallibrary.un.org", .)

html_r4 <- read_html(url_global4) %>%
  html_nodes('a') %>%
   html_attr('href') %>%
    grep('EN.pdf', ., value=TRUE) %>%
     str_c("https://digitallibrary.un.org", .)
...
# get the two xml files to be compared from global_url
xtml_r1 <- read_xml(url(url_global1), as_html=TRUE)

xtml_r1 <- read_xml(curl::curl(url_global1), as_html=TRUE)

xtml_r2 <- read_xml(url(url_global2), as_html=TRUE)
 
a = xmlParse(xtml_r1, asText = TRUE)
b = xmlParse(xtml_r2, asText = TRUE)


temp <- compareXMLDocs(xtml_r1, xtml_r2)
...
#==========================
# (7) GEOGRAPHICAL CATEGORY ####
#==========================

# remov


## Idee: specific meta-information related to a resolution
#
#  (1) Authr_land
#  (2) Year
#  (3) Location
